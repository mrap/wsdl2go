...
func (ge *goEncoder) writeGoFuncs(w io.Writer, d *wsdl.Definitions) error {
	...
	for _, fn := range ge.funcnames {
		op := ge.funcs[fn]
		name := goID(op.Name, true)
		ge.writeComments(w, name, op.Doc)
		inParams, err := ge.inputParams(op)
		if err != nil {
			return err
		}
		...
		if !ok {
			...
			fn := ge.fixFuncNameConflicts(name)
...
func (ge *goEncoder) wsdl2goType(t string) string {
	...
	case "anysequence":
		return "interface{}"
	default:
		return "*" + goID(strings.Replace(v, ".", "", -1), true)
	}
}
...
func (ge *goEncoder) genElementField(w io.Writer, el *wsdl.Element) {
	...
	tag := el.Name
	fmt.Fprintf(w, "%s ", goID(el.Name, true))
	if el.Max != "" && el.Max != "1" {
		fmt.Fprintf(w, "[]")
		if slicetype != "" {
			tag += ">" + slicetype
...
func goID(s string, isPublic bool) string {
	if len(s) == 0 {
		return s
	}

	s = unhyphen(strings.Title(s))
	if !isPublic {
		return lowerFirst(s)
	} else {
		return s
